        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1.5">
            {['', 'ordered', 'received', 'cancelled'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === s ? 'bg-slate-800 text-white' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}>
                {s ? s.charAt(0).toUpperCase() + s.slice(1) : 'All'}
              </button>
            ))}
          </div>
          <Button onClick={() => setShowModal(true)} className="h-9 px-3 rounded-lg bg-slate-800 hover:bg-slate-900 text-sm" data-testid="create-po-btn">
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="rounded-2xl max-w-xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleCreate} className="rounded-lg bg-slate-800 hover:bg-slate-900" data-testid="confirm-po-btn">Create Order</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
