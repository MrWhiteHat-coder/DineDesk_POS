    ...
    <div className="flex flex-col lg:flex-row gap-3 lg:gap-4 lg:h-[calc(100vh-7rem)]" data-testid="pos-main">
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:min-h-0">
        ...
        {runningOrders.length > 0 && (
          <div className="mb-3 flex items-center gap-2 overflow-x-auto pb-1">
            ...
            {runningOrders.map(ro => (
              <button key={ro.id} onClick={() => selectRunningOrder(ro)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap border transition-all ${selectedRunningOrder?.id === ro.id ? 'bg-yellow-400 text-neutral-900 border-black' : 'bg-slate-50 text-slate-600 hover:bg-slate-50'}`} data-testid={`running-order-${ro.id}`}>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2 md:gap-4 mb-3 md:mb-4">
          <div className="flex items-center gap-2 overflow-x-auto flex-1 pb-1 w-full md:w-auto">
            <button onClick={() => setSelectedCategory(null)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${!selectedCategory ? 'bg-yellow-400 text-neutral-900' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`} data-testid="category-all">
              All <span className={`text-xs px-1.5 py-0.5 rounded-md ${!selectedCategory ? 'bg-white/20' : 'bg-gray-100'}`}>{menuItems.length}</span>
            </button>
            {categories.map(cat => (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${selectedCategory === cat.id ? 'bg-yellow-400 text-neutral-900' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`} data-testid={`category-${cat.name.toLowerCase().replace(/\s+/g, '-')}`}>
                ...
                return (
                  <div key={item.id} className="bg-white rounded-xl overflow-hidden border border-slate-100 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5" data-testid={`menu-item-${item.id}`}>
                    <div className="relative aspect-[4/3] bg-slate-100">
                      ...
                      {qty > 0 && <span className="absolute top-2.5 left-2.5 w-6 h-6 bg-yellow-400 text-neutral-900 rounded-full text-[11px] font-bold flex items-center justify-center">{qty}</span>}
                    </div>
                    <div className="p-3">
                      ...
                      {!item.is_available ? (
                        ...
                        <button onClick={() => addToCart(item)} className="w-full flex items-center justify-center gap-1.5 py-2 rounded-lg bg-yellow-400 text-neutral-900 hover:bg-yellow-300 transition-colors" data-testid={`add-item-${item.id}`}><Plus className="w-3.5 h-3.5" /> Add to Cart</button>
                      )}
                    </div>
                  </div>
                );
      ...
      <div className="w-full lg:w-[320px] flex flex-col bg-white rounded-xl border border-slate-200 overflow-hidden lg:flex-shrink-0 lg:h-full">
        ...
        <div className="px-4 py-3 border-t border-slate-100 space-y-3">
          ...
          {cart.length > 0 && (
            <Button onClick={handlePlaceOrder} className="w-full h-11 rounded-lg bg-yellow-400 hover:bg-yellow-300 text-white font-semibold text-sm" disabled={!isDayOpen || checkoutLoading} data-testid="place-order-btn">
      ...
      <Dialog open={showReceipt} onOpenChange={setShowReceipt}>
        <DialogContent className="rounded-2xl max-w-xs" data-testid="receipt-modal">
          ...
          <button onClick={handlePrintReceipt} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-yellow-400 text-neutral-900 hover:bg-yellow-300 transition-colors mt-2" data-testid="print-receipt-btn"><Printer className="w-4 h-4" /> Print Receipt</button>
        </DialogContent>
      </Dialog>
    </div>
  );
